...
- Latest shipped feature baseline: Stage 5A5 review helper split on top of Stage 5A4 low-risk `mindmap.js` module split, Stage 5A3 runtime-to-portable snapshot parity fixtures, Stage 5A2 mindmap runtime CSS/JS extraction, Stage 5A1 pure portable contract/model/review helpers, and the Stage 5A0 compatibility audit. For the exact current commit, run `git rev-parse main`.
