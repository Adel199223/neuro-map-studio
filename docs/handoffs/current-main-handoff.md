...
- Map editor route at `public/prototypes/current/mindmap.html?pageId=<id>`, with the prototype stylesheet in `public/prototypes/current/mindmap.css`, the browser entrypoint in `public/prototypes/current/mindmap.js`, and low-risk constants/string/geometry/review/storage/relationship helpers in sibling `mindmap*.js` modules
