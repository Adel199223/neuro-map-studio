...
- Latest shipped feature baseline: Stage 5A6 storage helper split on top of Stage 5A5 review helper split, Stage 5A4 low-risk `mindmap.js` module split, Stage 5A3 runtime-to-portable snapshot parity fixtures, Stage 5A2 mindmap runtime CSS/JS extraction, Stage 5A1 pure portable contract/model/review helpers, and the Stage 5A0 compatibility audit. For the exact current commit, run `git rev-parse main`.
...
- Map editor route at `public/prototypes/current/mindmap.html?pageId=<id>`, with the prototype stylesheet in `public/prototypes/current/mindmap.css`, the browser entrypoint in `public/prototypes/current/mindmap.js`, and low-risk constants/string/geometry/review helpers in sibling `mindmap*.js` modules
