...
Pure review helper logic now lives in `public/prototypes/current/mindmapReviewHelpers.js`, low-risk storage helper logic now lives in `public/prototypes/current/mindmapStorageHelpers.js`, pure relationship data helpers now live in `public/prototypes/current/mindmapRelationshipHelpers.js`, and pure menu descriptor helpers now live in `public/prototypes/current/mindmapMenuHelpers.js`, while `mindmap.js` still owns runtime state, rendering, menu DOM/event wiring, DOM-bound storage wiring, and review UI wiring. Stage 5A1/5A3 TypeScript helpers remain unwired from the browser runtime.
