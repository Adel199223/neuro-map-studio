...
Pure review helper logic now lives in `public/prototypes/current/mindmapReviewHelpers.js`, low-risk storage helper logic now lives in `public/prototypes/current/mindmapStorageHelpers.js`, and pure relationship data helpers now live in `public/prototypes/current/mindmapRelationshipHelpers.js`, while `mindmap.js` still owns runtime state, rendering, DOM-bound storage wiring, and review UI wiring. Stage 5A1/5A3 TypeScript helpers remain unwired from the browser runtime.
